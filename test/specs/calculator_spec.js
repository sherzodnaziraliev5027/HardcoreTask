...
describe("calculator suite",() =>{
    ...
    await calculatorPage.numberOfInstances.waitForDisplayed({timeout:7000})
    await calculatorPage.numberOfInstances.setValue(4);
    await calculatorPage.instancesfor.setValue('leave blank');
    await calculatorPage.clickSelection.waitForClickable({timeout:5000})
    ...
    await calculatorPage.selectN1Standart8GbGpu.waitForClickable({timeout:5000})
    await calculatorPage.selectN1Standart8GbGpu.click()
    await calculatorPage.makeCheckedAddGpus.waitForClickable({timeout:5000})
    await calculatorPage.makeCheckedAddGpus.click();
    await calculatorPage.makeunCheckedDiscounts.click()
    await calculatorPage.clickGpuType.waitForClickable({timeout:5000})
    await calculatorPage.clickGpuType.click()
    await calculatorPage.selectGpuType.waitForDisplayed({timeout:5000})
    await calculatorPage.selectGpuType.click()
    await calculatorPage.clickNumberofGpus.waitForClickable({timeout:5000})
    await calculatorPage.clickNumberofGpus.click()
    await calculatorPage.selectNumberofGpus.waitForDisplayed({timeout:5000})
    await calculatorPage.selectNumberofGpus.click()
    await calculatorPage.clickLocalSSd.waitForClickable({timeout:5000})
    await calculatorPage.clickLocalSSd.click()
    await calculatorPage.selectLocalSSd.click()
    await calculatorPage.clickDataCenterLocation.waitForClickable({timeout:5000})
    await calculatorPage.clickDataCenterLocation.click();
    await calculatorPage.selectDataCenterLocation.waitForDisplayed({timeout:5000});
    ...
    await estimateCard.emailPageButton.click()
})
it("Copying email from new window and check cost",async() =>{
    ...
    await newWindowPage.copyEmail.click()
    ...
    await browser.switchToWindow(handles[0])
    await estimateCard.estimatedCost.waitForDisplayed({timeout:5000})
