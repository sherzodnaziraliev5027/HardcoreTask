...
it("Copying email from new window and check cost",async() =>{
    ...
    await newWindowPage.emailInput.waitForDisplayed({timeout:3000});
    await newWindowPage.copyEmail
    const handles = await browser.getWindowHandles()
    await browser.switchToWindow(handles[0])
    await emailPage.emailInput.waitForDisplayed({timeout:3000})
    await emailPage.emailInput.click()
    await emailPage.pasteEmail
    await emailPage.sendButton.waitForClickable({timeout:3000})
